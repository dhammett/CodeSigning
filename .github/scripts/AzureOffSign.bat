...
set azuresigntool=azuresigntool.exe
...
set E_SIGN_NOT_FOUND="Failed when searching azuresigntool.exe."
...
IF EXIST %signtoolPath% (
	...
	echo %signtoolPath%
	IF EXIST %azuresigntoolPath% (
		echo Successfully found the location of azuresigntool.exe:
		echo %azuresigntoolPath%
...
echo.
call %azuresigntoolPath% %signCmd%
if %errorlevel%==0 (
	goto LRun1stVerify
) else (
...
:LUsage
echo.
echo AzureOffSign.bat -- signing and verification of signatures for VBA projects contained in Office files.
echo.
echo Usage:
echo     AzureOffSign.bat "Signtool.exe location" "Signing command" "Verify command" "filename"
echo.
echo Example:
echo     AzureOffSign.bat "C:\Program Files (x86)\Windows Kits\10\bin\10.0.18362.0\x86\" "sign /f D:\CodeSign.pfx /fd SHA256" "verify /pa" "D:\TestFile.xlsm"
echo.
echo Error return:
